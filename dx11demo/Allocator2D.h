...
namespace MeshDescriptionOp
{
	...
	public:
		...
		struct FRow
		{
			...
			std::vector< FSegment > FreeSegments;
			std::vector< FSegment > UsedSegments;
		};
	...
	inline uint64 FAllocator2D::GetBit(uint32 x, uint32 y) const
	{
		return Bits[(x >> 6) + y * Pitch] & (1ull << (x & 63));
	}

	inline void FAllocator2D::SetBit(uint32 x, uint32 y)
	{
		Bits[(x >> 6) + y * Pitch] |= (1ull << (x & 63));
	}

	inline void FAllocator2D::ClearBit(uint32 x, uint32 y)
	{
		Bits[(x >> 6) + y * Pitch] &= ~(1ull << (x & 63));
	}
