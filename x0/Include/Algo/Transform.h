...
namespace Algo
{
	...
	*/
	...
	inline void TransformIf(const InT& Input, OutT& Output, PredicateT Predicate, TransformT Trans)
	{
	...
	*/
	...
	inline void Transform(const InT& Input, OutT& Output, TransformT Trans)
	{
		for (const auto& Value : Input)
		{
			Output.Add(Invoke(Trans, Value));
