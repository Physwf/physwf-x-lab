...
public:
	...
	* @see ToString
	*/
	static X0_API bool Parse(const FString& TimespanString, FTimespan& OutTimespan);
...
public:
	...
	* @return The archive.
	*/
	//friend X0_API FArchive& operator<<(FArchive& Ar, FTimespan& Timespan);
	...
	* @return Hash value.
	*/
	friend X0_API uint32 GetTypeHash(const FTimespan& Timespan);
...
protected:
	...
	* @param FractionNano The fractional seconds (in nanosecond resolution).
	*/
	void X0_API Assign(int32 Days, int32 Hours, int32 Minutes, int32 Seconds, int32 FractionNano);
