...
*/
inline VectorRegister VectorReciprocalSqrtAccurate(const VectorRegister& Vec)
{
 ...
 */
inline VectorRegister VectorReciprocalAccurate(const VectorRegister& Vec)
{
	// Perform two passes of Newton-Raphson iteration on the hardware estimate
	//   x1 = x0 - f(x0) / f'(x0)
	//
...
*/
inline VectorRegister VectorSet_W1( const VectorRegister& Vector )
{
 ...
 * @return				VectorRegister with 4 FLOATs loaded from Ptr.
 */
inline VectorRegister VectorLoadSRGBA16N(const void* Ptr)
{
 ...
 * Doing this in inline not only means you introduce a branch per static, but you bloat the inlined code immensely.
...
*/
inline void VectorSinCos(VectorRegister* __restrict VSinAngles, VectorRegister* __restrict VCosAngles, const VectorRegister* __restrict VAngles)
{
	// Map to [-pi, pi]
	// X = A - 2pi * round(A/2pi)
