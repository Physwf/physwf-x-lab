...
public:
	...
	explicit Normal3<T>(const Vector3<T> &v) : x(v.x), y(v.y), z(v.z) { DOCHECK(!v.HasNaNs()); }
...
public:
	...
	void BoundingSphere(Point3<T> *c, Float *rad) const
	{
		*c = (pMin + pMax) / 2;
		*rad = Inside(*c, *this) ? Distance(*c, pMax) : 0;
	}
...
inline bool Bounds3<T>::IntersectP(const Ray& ray, Float *hitt0 /* = nullptr */, Float *hitt1 /* = nullptr */) const
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		//see page 220;
		//防止两个t的误差区间彼此重叠,将tmaxde值增加2 * gamma(3),从而避免重叠
		tFar *= 1 + 2 * gamma(3);
