...
struct Distribution1D
{
	...
	Float SampleContinuous(Float u, Float *pdf, int *off = nullptr) const {
		...
		if ((cdf[offset + 1] - cdf[offset]) > 0) {
			DOCHECK(cdf[offset + 1] > cdf[offset]);
			du /= (cdf[offset + 1] - cdf[offset]);
		}
		DOCHECK(!std::isnan(du));
	...
	int SampleDiscrete(Float u, Float *pdf = nullptr,
		...
		//if (uRemapped) DOCHECK(*uRemapped >= 0.f && *uRemapped <= 1.f);
		return offset;
	}
	Float DiscretePDF(int index) const {
		DOCHECK(index >= 0 && index < Count());
...
Vector3f UniformSamplerHemisphere(const Point2f& u);
Float UniformHemispherePdf();
Vector3f UniformSampleSphere(const Point2f& u);
Float UniformSpherePdf();
Vector3f UniformSampleCone(const Point2f& u, Float thetamax);
Vector3f UniformSampleCone(const Point2f& u, Float thetamax, const Vector3f& x, const Vector3f& y, const Vector3f &z);
...
template<typename T>
void Shuffle(T* samp, int count, int nDimentions, RNG &rng)
{
	for (int i = 0; i < count; ++i)
	{
		...
		{
			std::swap(samp[nDimentions*i + j], samp[nDimentions*other + j]);
		}
	}
}
