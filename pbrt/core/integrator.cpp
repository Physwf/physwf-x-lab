...
Spectrum UniformSampleAllLight(const Interaction & it, const Scene & scene, MemoryArena & arena, Sampler & sampler, const std::vector<int>& nLightSamples, bool handleMedia)
{
	Spectrum L(0.0f);
...
Spectrum EstimateDirect(const Interaction& it, const Point2f & uScattering, const Light& light, const Point2f& uLight, const Scene& scene, Sampler& sampler, MemoryArena& arena, bool handleMedia, bool specular)
{
	...
	VisibilityTester visibitity;
	Spectrum Li = light.Sample_Li(it, uLight, &wi, &lightPdf, &visibitity);
	if (lightPdf > 0 && !Li.IsBlack())
	{
		...
		{
			if (handleMedia)
				Li *= visibitity.Tr(scene, sampler);
			else if (!visibitity.Unoccluded(scene))
				Li = Spectrum(0.f);
			if (!Li.IsBlack())
			{
