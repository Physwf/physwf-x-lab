...
UVMapping2D::UVMapping2D(Float su /*= 1*/, Float sv /*= 1*/, Float du /*= 0*/, Float dv /*= 0*/)
...
Point2f PlanerMapping2D::Map(const SurfaceInteraction& si, Vector2f* dstdx, Vector2f* dstdy) const
{
	Vector3f vec(si.p);
...
Point3f IdentityMapping3D::Map(const SurfaceInteraction& si, Vector3f* dpdx, Vector3f* dpdy) const
{
