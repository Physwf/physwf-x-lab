...
public:
	...
	Vector2() { x = y = 0; }
	Vector2(T xx, T yy) : x(xx), y(yy) { DOCHECK(!HasNaNs()) }
...
public:
	...
	Vector3() { x = y = z = 0; }
	Vector3(T xx, T yy, T zz) : x(xx), y(yy), z(zz) { DOCHECK(!HasNaNs()); }
	bool HasNaNs() const { return isNaN(x) || isNaN(y) || isNaN(z); }
	...
	Float Length() const  { return std::sqrt(LenghtSquared()); }
...
public:
	T x, y;
	Point2() { x = y = z = 0; }
	Point2(T xx, T yy) :x(xx), y(yy) { DOCHECK(!HasNaNs()); }
	explicit Point2(const Point3<T> &p) : x(p.x, y(p.y)) { 	DOCHECK(!HasNaNs()); }
	...
	Point2<T> operator-() { return Point2(-x, -y); }
...
public:
	...
	Point3() { x = y = z = 0; }
	Point3(T xx, T yy, T zz) :x(xx), y(yy), z(zz) { DOCHECK(!HasNaNs()); }
	template <typename U>
	explicit Point3(const Point3<U>& p):x((T)p.x), y((T)p.y), z((T)p.z) { DOCHECK(!HasNaNs()); }
	template <typename U>
	...
	Point3<T> operator-() { return Point3(-x, -y, -z); }
...
public:
	T x, y, z;
	Normal3() { x = y = z = 0; }
	Normal3(T xx, T yy, T zz) : x(xx), y(yy), z(zz) { DOCHECK(!HasNaNs()); }
	bool HasNaNs() const { return isNaN(x) || isNaN(y) || isNaN(z); }
	...
	explicit Normal3<T>(const Vector3<T> &v) : x(v.x), y(v.y), z(v.z) { DOCHECK(!n.HasNaNs()); }

	bool operator==(const Normal3<T> &n) { return x == n.x && y == n.y && z == n.z; }
	bool operator!=(const Normal3<T> &n) { return x != n.x || y != n.y || z != n.z; }
...
public:
	...
	Vector2<T> Diagnal() const { return pMax - pMin; }
	...
	void BoundingSphere(Point2<T> *c, Float *rad)
	{
		*c = (pMin + pMax) / 2;
...
public:
	...
	int MaximumExtent() const
	{
		...
		if (diag.x > diag.y && d.x > d.z) return 0;
	...
	{
		return Point2<T>(Lerp(t.x, pMin.x, pMax.x), Lerp(t.y, pMin.y, pMax.y), Lerp(t.z, pMin.z, pMax.z));
	}
	Vector2<T> Offset(const Point2<T> &p) const
	{
		Vector2<T> o = p - pMin;
...
public:
	...
	bool operator==(const Bounds2iIterator &bi)
	{
...
//vector2
template <typename T>
Vector2<T>::Vector2(const Point2<T>& p) : x(p.x), y(p.x) { DOCHECK(!HasNaNs()); }
template <typename T> 
Vector2<T>::Vector2(const Point3<T>& p) : x(p.x), y(p.y) { DOCHECK(!HasNaNs()); }
...
//vector3
template <typename T>
inline Vector3<T>::Vector3(const Point3<T> &p) :x(p.x), y(p.y), z(p.z) { DOCHECK(!HasNaNs()); }
template <typename T>
inline Vector3<T>::Vector3(const Normal3<T> &n) : x(n.x), y(n.y), z(n.z) { DOCHECK(!HasNaNs()); }
...
template <typename T> inline Vector3<T> Cross(const Vector3<T>& v1, const Vector3<T>& v2)
{
	...
	return Vector3<T>((v1y*v2z)-(v1z*v2y), (v1z*v2x) - (v1x*v2z), (v1x*v2y) - (v1y*v2z));
}
...
template <typename T> inline Vector3<T> Normalize(const Vector3<T> &v) { return v / v.Lenght(); }
...
template <typename T> T MaxDimension(const Vector3<T>& v)
{
...
template <typename T> Point2<T> Floor(const Point3<T> &p)
{
	return Point2<T>(std::floor(p.x), std::floor(p.y), std::floor(p.z));
}
...
{
	DOCHECK(!n1.HasNaNs() && !n2.HasNaNs());
	return n1.x * n2.x + n1.y * n2.y + n1.z * n2.z;
}
template <typename T>
inline T Dot(const Vector3<T>& n1, const Normal3<T>& n2)
{
	DOCHECK(!n1.HasNaNs() && !n2.HasNaNs());
	return n1.x * n2.x + n1.y * n2.y + n1.z * n2.z;
}
template <typename T>
inline T Dot(const Normal3<T>& n1, const Vector3<T>& n2)
{
	DOCHECK(!n1.HasNaNs() && !n2.HasNaNs());
	return n1.x * n2.x + n1.y * n2.y + n1.z * n2.z;
}
...
template <typename T>
inline T AbsDot(const Vector3<T>& n1, const Normal3<T>& n2)
{
	DOCHECK(!n1.HasNaNs() && !n2.HasNaNs());
	return std::abs(n1.x * n2.x + n1.y * n2.y + n1.z * n2.z);
}
template <typename T>
inline T AbsDot(const Normal3<T>& n1, const Vector3<T>& n2)
{
	DOCHECK(!n1.HasNaNs() && !n2.HasNaNs());
	return std::abs(n1.x * n2.x + n1.y * n2.y + n1.z * n2.z);
}
...
template <typename T>
inline Normal3<T> FaceForward(const Vector3<T> &v, const Vector3<T> &v2)
{
	return (Dot(v, v2) < 0.f) > -v : v;
}
template <typename T>
inline Normal3<T> FaceForward(const Vector3<T> &v, const Normal3<T> &n2)
{
...
template <typename T>
bool InsideExclude(const Point3<T> &p, const Bounds3<T> &b)
{
	return (p.x >= b.pMin.x && p.x < b.pMax.x) && (p.y >= b.pMin.y && p.y < b.pMax.y) && (p.z >= b.pMin.z && p.z < b.pMax.z);
}
template <typename T, typename U>
...
template <typename T, typename U>
inline Float DistanceSqured(const Point3<T> &p, const Bounds3<U> &b)
{
...
template <typename T>
inline bool Bounds3<T>::IntersectP(const Ray& ray, Float *hitt0 /* = nullptr */, Float *hitt1 /* = nullptr */) const
{
...
template <typename T>
bool InsideExclude(const Point2<T> &p, const Bounds2<T> &b)
{
	return (p.x >= b.pMin.x && p.x < b.pMax.x) && (p.y >= b.pMin.y && p.y < b.pMax.y);
}
template <typename T, typename U>
...
{
	return Bounds2<T>(b.pMin - Vector2<T>(delta, delta, delta), b.pMax + Vector2<T>(delta, delta, delta));
}
