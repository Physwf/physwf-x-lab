...
#include "RenderCore/RenderCommandFence.h"
#include "RHI/RHI.h"
...
X4_API bool GIsThreadedRendering = false;
X4_API bool GUseThreadedRendering = false;

#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
X4_API TAtomic<bool> GMainThreadBlockedOnRenderThread;
...
FSuspendRenderingThread::FSuspendRenderingThread(bool bInRecreateThread)
{
	...
	if (/*IsAsyncLoadingMultithreaded()*/false)
	{
		//SuspendAsyncLoading();
	}
...
public:
	...
	static int32 FlushRHILogsAndReportCrash(Windows::LPEXCEPTION_POINTERS ExceptionInfo)
	{
		...
		return 0;// ReportCrash(ExceptionInfo);
	}
#endif
...
{
	return FString::Printf(TEXT("%s %u"), TEXT("RenderThread"), ThreadIndex);
}
...
void FRenderCommandFence::BeginFence(bool bSyncToRHIAndGPU)
{
	...
	else
	{
		...
		int32 GTSyncType = 0;// CVarGTSyncType.GetValueOnAnyThread();
		if (bSyncToRHIAndGPU)
		{
...
static void GameThreadWaitForTask(const FGraphEventRef& Task, bool bEmptyGameThreadTasks = false)
{
	...
	if (!Task->IsComplete())
	{
		//SCOPE_CYCLE_COUNTER(STAT_GameIdleTime);
		{
			...
			bool bRenderThreadEnsured = /*FDebug::IsEnsuring()*/ false;

			static bool bDisabled = false;// FParse::Param(FCommandLine::Get(), TEXT("nothreadtimeout"));
