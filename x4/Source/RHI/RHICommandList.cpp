...
X4_API bool GEnableAsyncCompute = true;
X4_API FRHICommandListExecutor GRHICommandList;
...
X4_API FRHICommandListFenceAllocator GRHIFenceAllocator;
...
// 	}
	...
	ENamedThreads::Type GetDesiredThread()
	{
		...
		return IsRunningRHIInDedicatedThread() ? ENamedThreads::RHIThread : ENamedThreads::NormalThreadPriority;// CPrio_RHIThreadOnTaskThreads.Get();
	}
...
// 	}
	...
	ENamedThreads::Type GetDesiredThread()
	{
		...
		check(IsRunningRHIInSeparateThread()); // this should never be used on a platform that doesn't support the RHI thread
		return bRHIThread ? (IsRunningRHIInDedicatedThread() ? ENamedThreads::RHIThread : ENamedThreads::NormalThreadPriority /*CPrio_RHIThreadOnTaskThreads.Get()*/) : ENamedThreads::GetRenderThread_Local();
	}
...
void FRHICommandListExecutor::ExecuteInner(FRHICommandListBase& CmdList)
{
	...
	if (IsRunningRHIInSeparateThread())
	{
		...
		if (bIsInRenderingThread)
		{
			...
			bAsyncSubmit = /*CVarRHICmdAsyncRHIThreadDispatch.GetValueOnRenderThread()*/ 1 > 0;
		...
		if (/*CVarRHICmdUseThread.GetValueOnRenderThread()*/ 1 > 0 && bIsInRenderingThread && !bIsInGameThread)
		{
			...
			if (/*CVarRHICmdForceRHIFlush.GetValueOnRenderThread()*/0 > 0)
			{
...
// 			}
		...
		bool NewBypass = IsInGameThread() || (/*CVarRHICmdBypass.GetValueOnAnyThread()*/ FRHICommandListExecutor::DefaultBypass  >= 1);
...
bool FRHICommandListExecutor::IsRHIThreadActive()
{
	...
	bool bAsyncSubmit = 1;// CVarRHICmdAsyncRHIThreadDispatch.GetValueOnRenderThread() > 0;
	if (bAsyncSubmit)
	{
		if (RenderThreadSublistDispatchTask.GetReference() && RenderThreadSublistDispatchTask->IsComplete())
		{
...
// 	}
	...
	ENamedThreads::Type GetDesiredThread()
	{
		return bIsPrepass ? ENamedThreads::NormalThreadPriority/* CPrio_FParallelTranslateCommandListPrepass.Get()*/ : ENamedThreads::NormalTaskPriority /*CPrio_FParallelTranslateCommandList.Get()*/;
	}
...
public:
		...
		, bIsPrepass(bInIsPrepass)
	{
		...
		MinSize = /*CVarRHICmdMinCmdlistSizeForParallelTranslate.GetValueOnRenderThread()*/ 32 * 1024;
		MinCount = /*CVarRHICmdMinCmdlistForParallelTranslate.GetValueOnRenderThread()*/ 2;
	}
...
// 	}
	...
	{
		return ENamedThreads::HighTaskPriority;// CPrio_FParallelTranslateSetupCommandList.Get();
	}
	...
	void DoTask(ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
	{
		...
		else
		{
			...
			while (Start < NumCommandLists)
			{
				...
				MyCompletionGraphEvent->SetGatherThreadForDontCompleteUntil(ENamedThreads::Type(/*CPrio_FParallelTranslateCommandList.Get() */ ENamedThreads::NormalThreadPriority | ENamedThreads::NormalTaskPriority | ENamedThreads::HighTaskPriority));
...
void FRHICommandListBase::QueueParallelAsyncCommandListSubmit(FGraphEventRef* AnyThreadCompletionEvents, bool bIsPrepass, FRHICommandList** CmdLists, int32* NumDrawsIfKnown, int32 Num, int32 MinDrawsPerTranslate, bool bSpewMerge)
{
	...
	if (/*CVarRHICmdFlushOnQueueParallelSubmit.GetValueOnRenderThread()*/0)
	{
	...
	{
		//static const auto ICVarRHICmdBalanceParallelLists = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.RHICmdBalanceParallelLists"));2

		if (/*ICVarRHICmdBalanceParallelLists->GetValueOnRenderThread()*/ 2 == 0 && /*CVarRHICmdBalanceTranslatesAfterTasks.GetValueOnRenderThread()*/0 > 0 && GRHISupportsParallelRHIExecute && /*CVarRHICmdUseDeferredContexts.GetValueOnAnyThread()*/1 > 0)
		{
			FGraphEventArray Prereq;
		...
		bool bMerge = !!/*CVarRHICmdMergeSmallDeferredContexts.GetValueOnRenderThread()*/0;
		int32 EffectiveThreads = 0;
		int32 Start = 0;
		int32 ThreadIndex = 0;
		if (GRHISupportsParallelRHIExecute && /*CVarRHICmdUseDeferredContexts.GetValueOnAnyThread()*/1 > 0)
		{
			// this is pretty silly but we need to know the number of jobs in advance, so we run the merge logic twice
			while (Start < Num)
			{
...
bool FRHICommandListImmediate::StallRHIThread()
{
	...
	bool bAsyncSubmit = /*CVarRHICmdAsyncRHIThreadDispatch.GetValueOnRenderThread()*/1 > 0;
	if (bAsyncSubmit)
	{
		if (RenderThreadSublistDispatchTask.GetReference() && RenderThreadSublistDispatchTask->IsComplete())
		{
...
void* FDynamicRHI::LockVertexBuffer_RenderThread(class FRHICommandListImmediate& RHICmdList, FVertexBufferRHIParamRef VertexBuffer, uint32 Offset, uint32 SizeRHI, EResourceLockMode LockMode)
{
	...
	check(IsInRenderingThread());
	bool bBuffer = /*CVarRHICmdBufferWriteLocks.GetValueOnRenderThread()*/ 1 > 0;
	void* Result;
...
{
	if (Bypass() || !IsRunningRHIInSeparateThread() || /*CVarRHICmdFlushUpdateTextureReference.GetValueOnRenderThread()*/ 0 > 0)
	{
		{
