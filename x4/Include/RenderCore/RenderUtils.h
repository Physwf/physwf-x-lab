...
extern X4_API FPixelFormatInfo GPixelFormats[PF_MAX];		// Maps members of EPixelFormat to a FPixelFormatInfo describing the format.
...
extern X4_API SIZE_T CalculateImageBytes(uint32 SizeX, uint32 SizeY, uint32 SizeZ, uint8 Format);

/** A global white texture. */
extern X4_API class FTexture* GWhiteTexture;

/** A global black texture. */
extern X4_API class FTexture* GBlackTexture;

/** A global black array texture. */
extern X4_API class FTexture* GBlackArrayTexture;

/** A global black volume texture. */
extern X4_API class FTexture* GBlackVolumeTexture;

/** A global black volume texture<uint>  */
extern X4_API class FTexture* GBlackUintVolumeTexture;

/** A global white cube texture. */
extern X4_API class FTexture* GWhiteTextureCube;

/** A global black cube texture. */
extern X4_API class FTexture* GBlackTextureCube;

/** A global black cube depth texture. */
extern X4_API class FTexture* GBlackTextureDepthCube;

/** A global black cube array texture. */
extern X4_API class FTexture* GBlackCubeArrayTexture;

/** A global texture that has a different solid color in each mip-level. */
extern X4_API class FTexture* GMipColorTexture;

/** Number of mip-levels in 'GMipColorTexture' */
extern X4_API int32 GMipColorTextureMipLevels;

// 4: 8x8 cubemap resolution, shader needs to use the same value as preprocessing
extern X4_API const uint32 GDiffuseConvolveMipLevel;

/** The indices for drawing a cube. */
extern X4_API const uint16 GCubeIndices[12 * 3];
 ...
 */
X4_API SIZE_T CalcTextureMipMapSize3D(uint32 TextureSizeX, uint32 TextureSizeY, uint32 TextureSizeZ, EPixelFormat Format, uint32 MipIndex);
 ...
 */
X4_API void CalcMipMapExtent3D(uint32 TextureSizeX, uint32 TextureSizeY, uint32 TextureSizeZ, EPixelFormat Format, uint32 MipIndex, uint32& OutXExtent, uint32& OutYExtent, uint32& OutZExtent);
 ...
 * @param MipIndex	The index of the mip-map to compute the size of.
 */
X4_API FIntPoint CalcMipMapExtent(uint32 TextureSizeX, uint32 TextureSizeY, EPixelFormat Format, uint32 MipIndex);
 ...
 * @param MipIndex			The index of the mip-map to compute the size of.
 */
X4_API SIZE_T CalcTextureMipWidthInBlocks(uint32 TextureSizeX, EPixelFormat Format, uint32 MipIndex);
 ...
 * @param MipIndex			The index of the mip-map to compute the size of.
 */
X4_API SIZE_T CalcTextureMipHeightInBlocks(uint32 TextureSizeY, EPixelFormat Format, uint32 MipIndex);
 ...
 * @param MipIndex	The index of the mip-map to compute the size of.
 */
X4_API SIZE_T CalcTextureMipMapSize(uint32 TextureSizeX, uint32 TextureSizeY, EPixelFormat Format, uint32 MipIndex);
 ...
 */
X4_API SIZE_T CalcTextureSize(uint32 SizeX, uint32 SizeY, EPixelFormat Format, uint32 MipCount);
 ...
 * @param MipCount	Number of mip-levels (including the base mip-level)
 */
X4_API SIZE_T CalcTextureSize3D(uint32 SizeX, uint32 SizeY, uint32 SizeZ, EPixelFormat Format, uint32 MipCount);
 ...
 */
X4_API void CopyTextureData2D(const void* Source, void* Dest, uint32 SizeY, EPixelFormat Format, uint32 SourceStride, uint32 DestStride);
 ...
 */
X4_API const TCHAR* GetPixelFormatString(EPixelFormat InPixelFormat);
/**
 * string to enum (not case sensitive)
 *
 * @param InPixelFormatStr e.g. "PF_B8G8R8A8", must not not be 0
 */
X4_API EPixelFormat GetPixelFormatFromString(const TCHAR* InPixelFormatStr);
 ...
 */
X4_API const TCHAR* GetCubeFaceName(ECubeFace Face);
 ...
 */
X4_API ECubeFace GetCubeFaceFromName(const FString& Name);

X4_API FVertexDeclarationRHIRef& GetVertexDeclarationFVector4();

X4_API FVertexDeclarationRHIRef& GetVertexDeclarationFVector3();

X4_API bool PlatformSupportsSimpleForwardShading(EShaderPlatform Platform);

X4_API bool IsSimpleForwardShadingEnabled(EShaderPlatform Platform);

inline bool IsForwardShadingEnabled(ERHIFeatureLevel::Type FeatureLevel)
{
	extern X4_API int32 bUseForwardShading;
...
X4_API FVertexBufferRHIRef& GetUnitCubeVertexBuffer();

/** Unit cube index buffer */
X4_API FIndexBufferRHIRef& GetUnitCubeIndexBuffer();
...
*/
X4_API void QuantizeSceneBufferSize(const FIntPoint& InBufferSize, FIntPoint& OutBufferSize);