...
class X4_API FUniformBufferStruct
{
public:
	...
	/** Initialization constructor. */
	FUniformBufferStruct(const FString& InLayoutName, const TCHAR* InStructTypeName, const TCHAR* InShaderVariableName, ConstructUniformBufferParameterType InConstructRef, uint32 InSize, const TArray<FMember>& InMembers, bool bRegisterForAutoBinding);

	virtual ~FUniformBufferStruct()
	{
		GlobalListLink.Unlink();
		GetNameStructMap().Remove(FString(StructTypeName));
	}
	...
	static TMap<FString, FUniformBufferStruct*>& GetNameStructMap();
...
extern X4_API FUniformBufferStruct* FindUniformBufferStructByName(const TCHAR* StructName);
extern X4_API FUniformBufferStruct* FindUniformBufferStructByFName(FName StructName);