...
struct X4_API FPipelineFileCacheRasterizerState
{
...
// 	}
	...
	friend X4_API uint32 GetTypeHash(const FPipelineFileCacheRasterizerState &Key)
	{
 ...
 */
struct X4_API FPipelineStateStats
{
	FPipelineStateStats()
		: FirstFrameUsed(-1)
		, LastFrameUsed(-1)
	...
	//friend X4_API FArchive& operator<<(FArchive& Ar, FPipelineStateStats& Info);
...
struct X4_API FPipelineCacheFileFormatPSO
{
	struct X4_API ComputeDescriptor
	{
	...
	struct X4_API GraphicsDescriptor
	{
	...
	friend X4_API uint32 GetTypeHash(const FPipelineCacheFileFormatPSO &Key);
	//friend X4_API FArchive& operator<<(FArchive& Ar, FPipelineCacheFileFormatPSO& Info);
...
struct X4_API FPipelineCachePSOHeader
{
	TSet<FSHAHash> Shaders;
	uint32 Hash;
};

extern X4_API const uint32 FPipelineCacheFileFormatCurrentVersion;
 ...
 */
class X4_API FPipelineFileCache
{
