...
inline uint32 ComputeAnisotropyRT(int32 InitializerMaxAnisotropy)
{
	...
	return FMath::Clamp(InitializerMaxAnisotropy > 0 ? InitializerMaxAnisotropy : /*CVarValue*/10, 1, 16);
}
...
class X4_API FDumpTransitionsHelper
{
public:
	static void DumpResourceTransition(const FString& ResourceName, const EResourceTransitionAccess TransitionType);
...
private:
	...
	static FString DumpTransitionForResource;
};
...
extern X4_API void SetDepthBoundsTest(FRHICommandList& RHICmdList, float WorldSpaceDepthNear, float WorldSpaceDepthFar, const FMatrix& ProjectionMatrix);

/** Returns the value of the rhi.SyncInterval CVar. */
extern X4_API uint32 RHIGetSyncInterval();
...
extern X4_API void RHIGetPresentThresholds(float& OutTopPercent, float& OutBottomPercent);

/** Signals the completion of the specified task graph event when the given frame has flipped. */
extern X4_API void RHICompleteGraphEventOnFlip(uint64 PresentIndex, FGraphEventRef Event);

extern X4_API void RHIInitializeFlipTracking();
extern X4_API void RHIShutdownFlipTracking();
...
extern X4_API FRHILockTracker GRHILockTracker;