...
public:
	...
	X4_API bool FindParameterAllocation(const TCHAR* ParameterName, uint16& OutBufferIndex, uint16& OutBaseIndex, uint16& OutSize) const;
	X4_API bool ContainsParameterAllocation(const TCHAR* ParameterName) const;
	X4_API void AddParameterAllocation(const TCHAR* ParameterName, uint16 BufferIndex, uint16 BaseIndex, uint16 Size);
	X4_API void RemoveParameterAllocation(const TCHAR* ParameterName);
	/** Checks that all parameters are bound and asserts if any aren't in a debug build
	* @param InVertexFactoryType can be 0
	*/
	X4_API void VerifyBindingsAreComplete(const TCHAR* ShaderTypeName, FShaderTarget Target, class FVertexFactoryType* InVertexFactoryType) const;
...
struct FShaderCompilerError
{
	...
	FString X4_API GetShaderSourceFilePath() const;
...
struct FShaderCompilerOutput
{
	...
	X4_API void GenerateOutputHash();
...
*/
extern X4_API bool CheckVirtualShaderFilePath(const FString& VirtualPath, TArray<FShaderCompilerError>* CompileErrors = nullptr);
...
*/
extern X4_API FString ParseVirtualShaderFilename(const FString& InFilename);
...
*/
extern X4_API bool LoadShaderSourceFile(const TCHAR* VirtualFilePath, FString& OutFileContents, TArray<FShaderCompilerError>* OutCompileErrors);

/** Loads the shader file with the given name.  If the shader file couldn't be loaded, throws a fatal error. */
extern X4_API void LoadShaderSourceFileChecked(const TCHAR* VirtualFilePath, FString& OutFileContents);

/**
* Recursively populates IncludeFilenames with the include filenames from Filename
*/
extern X4_API void GetShaderIncludes(const TCHAR* EntryPointVirtualFilePath, const TCHAR* VirtualFilePath, TArray<FString>& IncludeVirtualFilePaths, uint32 DepthLimit = 100);
...
*/
extern X4_API const class FSHAHash& GetShaderFileHash(const TCHAR* VirtualFilePath);

/**
* Calculates a Hash for the list of filenames if it does not already exist in the Hash cache.
*/
extern X4_API const class FSHAHash& GetShaderFilesHash(const TArray<FString>& VirtualFilePaths);
...
*/
extern X4_API void FlushShaderFileCache();

extern X4_API void VerifyShaderSourceFiles();
...
extern X4_API void SerializeUniformBufferInfo(class FShaderSaveArchive& Ar, const TMap<const TCHAR*, FCachedUniformBufferDeclaration>& UniformBufferEntries);