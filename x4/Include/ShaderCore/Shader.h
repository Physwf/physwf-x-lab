...
extern X4_API FSHAHash GGlobalShaderMapHash;
...
public:
	...
	//X4_API void Serialize(FArchive& Ar);

	// Reference counting.
	X4_API void AddRef();
	X4_API void Release();

	X4_API void Register();
	...
	X4_API FShaderResourceId GetId() const;
	...
	X4_API static FShaderResource* FindShaderResourceById(const FShaderResourceId& Id);

	/**
	* Finds a matching shader resource in memory or creates a new one with the given compiler output.
	* SpecificType can be NULL
	*/
	X4_API static FShaderResource* FindOrCreateShaderResource(const FShaderCompilerOutput& Output, class FShaderType* SpecificType, int32 SpecificPermutationId);

	/** Return a list of all shader Ids currently known */
	X4_API static void GetAllShaderResourceId(TArray<FShaderResourceId>& Ids);

	/** Returns true if and only if TargetPlatform is compatible for use with CurrentPlatform. */
	X4_API static bool ArePlatformsCompatible(EShaderPlatform CurrentPlatform, EShaderPlatform TargetPlatform);
	...
	*/
	X4_API static FSHAHash &FilterShaderSourceHashForSerialization(const FArchive& Ar, FSHAHash &HashToSerialize);
...
private:
	...
	*/
	...
	X4_API void InitializeShaderRHI();
...
public:
	...
	*/
	...
	X4_API FShaderId(const FSHAHash& InMaterialShaderMapHash, const FShaderPipelineType* InShaderPipeline, FVertexFactoryType* InVertexFactoryType, FShaderType* InShaderType, int32 PermutationId, FShaderTarget InTarget);
...
public:
	...
	*/
	...
	X4_API FSelfContainedShaderId();

	X4_API FSelfContainedShaderId(const FShaderId& InShaderId);

	X4_API bool IsValid();

	//X4_API friend FArchive& operator<<(FArchive& Ar, class FSelfContainedShaderId& Ref);
};

class FMaterial;

/** A compiled shader and its parameter bindings. */
class X4_API FShader : public FDeferredCleanupInterface
{
...
public:
	...
	*/
	...
	inline const FString& GetFName() const
	{
		return TypeName;
	}
...
public:
	...
	FORCEINLINE FString GetFName() const { return TypeName; }
...
public:
	...
	*/
	...
	void GetShaderList(TMap<FString, FShader*>& OutShaders) const
	{
		check(bHasBeenRegistered);
...
*/
extern X4_API void DumpShaderStats(EShaderPlatform Platform, EShaderFrequency Frequency);

/**
* Dumps shader pipeline stats to the log. Does not include material (eg shader pipeline instance) information.
* @param Platform  - Platform to dump shader info for, use SP_NumPlatforms for all
*/
extern X4_API void DumpShaderPipelineStats(EShaderPlatform Platform);
...
*/
extern X4_API FShaderType* FindShaderTypeByName(FName ShaderTypeName);

/** Helper function to dispatch a compute shader while checking that parameters have been set correctly. */
extern X4_API void DispatchComputeShader(
...
extern X4_API void DispatchIndirectComputeShader(
	FRHICommandList& RHICmdList,
	FShader* Shader,
...
extern X4_API const TArray<FName>& GetTargetShaderFormats();

/** Appends to KeyString for all shaders. */
extern X4_API void ShaderMapAppendKeyString(EShaderPlatform Platform, FString& KeyString);