...
extern X4_API const int32 GlobalShaderMapId;
...
public:
	...
	X4_API FGlobalShaderMapId(EShaderPlatform Platform);

	/** Append to a string that will be used as a DDC key. */
	X4_API void AppendKeyString(FString& KeyString) const;
...
extern X4_API TShaderMap<FGlobalShaderType>* GGlobalShaderMap[SP_NumPlatforms];
...
public:
	...
	X4_API FGlobalShader(const ShaderMetaType::CompiledShaderInitializerType& Initializer);
	...
	// FShader interface.
	X4_API static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment) {}
	X4_API static bool ValidateCompiledResult(EShaderPlatform Platform, const FShaderParameterMap& ParameterMap, TArray<FString>& OutErrors) { return true; }
};
...
{
	DECLARE_EXPORTED_SHADER_TYPE(FNULLPS, Global, X4_API);
public:
...
extern X4_API void BackupGlobalShaderMap(FGlobalShaderBackupData& OutGlobalShaderBackup);

/** Recreates shaders in the global shader map from the serialized memory. */
extern X4_API void RestoreGlobalShaderMap(const FGlobalShaderBackupData& GlobalShaderData);
...
*/
extern X4_API TShaderMap<FGlobalShaderType>* GetGlobalShaderMap(EShaderPlatform Platform);
