...
std::shared_ptr<Scene> BuildTestScene()
{
	...
	Transform ballT = Transform::Translate(0, 0/*-75.f*/, 0);
	std::shared_ptr<Material> ballMat = std::make_shared<MatteMaterial>(gray);
	std::shared_ptr<Shape> sphere = std::make_shared<Sphere>(50.f);
...
void Test_PathTracing()
{
	...
	std::shared_ptr<Sampler> sampler = Sampler::CreateStratified(1,1,8);
